...
# By default this resolves to the esmini v2.29.3 binaries bundled with
# commonroad-openscenario-converter, so the resource/catalog paths referenced by
# the .xosc scenarios resolve correctly and the co-simulation runs against the
# same engine build the converter itself uses.  OSC2CR_ESMINI_HOME overrides it.
_LIB = paths.esmini_lib("libesminiLib.so")

#: Absolute path to libesminiLib.so.  Kept as a plain string (not a Path) because
#: it is handed straight to ctypes.CDLL and compared against in the tests.  It is
#: the empty string when no esmini installation was found — loading then fails
#: with a clear OSError rather than silently running without a scenario engine.
ESMINI_LIB_PATH = str(_LIB) if _LIB else ""
_ESMINI_BIN_DIR = os.path.dirname(ESMINI_LIB_PATH)
